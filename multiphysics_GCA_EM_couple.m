...
epsilon_a_i=imag(epsilon_a);
epsilon_c = N_GST_c^2;%dielectric constant of c-GST
epsilon_c_i=imag(epsilon_c);
...
L_zh_totalz=0.6e-6;
model=createpde(1);
gm=multicuboid([L_zh_totalx],[L_zh_totaly],[L_zh_totalz],'Zoffset',-L_zh_totalz/2);
model.Geometry=gm;
applyBoundaryCondition(model,'dirichlet','Face',[1,2],'h',1,'r',0);
applyBoundaryCondition(model,'dirichlet','Face',[6],'h',1,'r',@incident_EM);

generateMesh(model,'Hmax',0.08e-6);

%pdeplot3D(model,'ColorMapData',abs(u(:,1)))
...
while(1)
   ...
   %% heat calculation
    ...
    if t_total_heat>heat_cal_count
        heat_cal_count=heat_cal_count+delta_heat_cal;
        specifyCoefficients(model,'m',0,'d',0,'c',-1,'a',@episilon_EM,'f',0);
        %pdegplot(model,'FaceLabels','on','FaceAlpha',0.5);
        %pdeplot3D(model);
        results = solvepde(model);
        u = results.NodalSolution;
        [Xp,Yp,Zp]=meshgrid(linspace(-500e-9,500e-9,N),linspace(-500e-9,500e-9,N),110e-9);
        E = interpolateSolution(results,Xp,Yp,Zp);
        E=reshape(E,size(Xp));
        Q=imag(((1-r)*epsilon_a+r*epsilon_c)).*abs(E).*abs(E);
        Q=Q/sum(sum(Q))*sum(sum(I))*1000;                
        max(max(Q))
    end
    %GST top layer
...
function bcMatrix = incident_EM(location,state)
xcenter=0;
sigmax=0.25e-6/4;
zcenter=0;
sigmaz=0.11e-6/4;
bcMatrix = [exp(-(location.x-xcenter).^2/(2*sigmax^2)+-(location.z-zcenter).^2/(2*sigmaz^2));
    ]; % OK to vectorize
end

function a = episilon_EM(location,state)
lambda=663e-9;
k0square=(2*pi/lambda)^2;
epair=1;
% domain 1
epis1=N_SiN^2;
L1xmin=-500e-9;L1xmax=500e-9;L1zmin=-100e-9;L1zmax=100e-9;
% domain 2
%Phasefield({0,0});
%size(location.x)
%T_GCA = F_interp({x_GCA,y_GCA});

Phasefield = griddedInterpolant(X_GCA-N*D/2,Y_GCA-N*D/2,r);%temperatur interpolation
epis2=Phasefield(location.x,location.y);
L2xmin=-500e-9;L2xmax=500e-9;L2zmin=100e-9;L2zmax=130e-9;
% use parameter
q=((location.x>L1xmin).*(location.x<L1xmax).*(location.z>L1zmin).*(location.z<L1zmax)).*(epis1-epair)+...
((location.x>L2xmin).*(location.x<L2xmax).*(location.z>L2zmin).*(location.z<L2zmax)).*(epis2-epair)+...
epair;
a=q*k0square;
a=complex(a);
end
